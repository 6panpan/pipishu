...
class AlbumService extends Service {
    ...
    async getRankAlbum(kind) {
        let sql =
            "select * from album,user where u_id=user_id and kind=? order by album_amount desc limit 0,10;";
        let r = await this.ctx.app.mysql.query(sql, [kind]);
        return r;
    }
    ...
    async getAllAlbumByKind(kind) {
        let sql =
            "select * from album,user where u_id=user_id and kind=? order by album_amount desc limit 0,100;";
        let r = await this.ctx.app.mysql.query(sql, [kind]);
        return r;
    }
    //ZY//获取某用户上传专辑
